...
export default function PipelineClient({ deals: initial }: Props) {
  ...
  const dealsByStage = useCallback((stage: string) =>
  ...
  async function onDragEnd({ active, over }: DragEndEvent) {
    ...
    // Detect target stage from column id or deal id
    let targetStage = STAGES.find(s => s.key === over.id)?.key
    if (!targetStage) {
              ...
              {STAGES.map(stage => {
                ...
                return (
                  <div key={stage.key} className="pipeline-col">
                    ...
                    >
                      <div
                        className={`col-body ${overId === stage.key ? 'drag-over' : ''}`}
                        id={stage.key}
                        style={{ minHeight: 200 }}
                      >
                        {stageDeals.map(deal => (
